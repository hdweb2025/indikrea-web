...
// Security Check:
// - Superadmin: boleh semua aksi
// - update_password: boleh oleh pemilik akun (id sama), dengan verifikasi password lama
...
try {
    switch ($action) {
        ...
        case 'update_website_usage':
            updateWebsiteUsage($pdo, $payload);
            break;
        ...
        case 'update_password':
            updatePassword($pdo, $input->user, $payload);
            break;
        default:
            http_response_code(400);
...
function updateWebsiteUsage($pdo, $payload) {
    if (!isset($payload->id)) {
        http_response_code(400);
        echo json_encode(['success' => false, 'message' => 'Missing website ID.']);
        return;
    }
    $fields = [];
    $params = [':id' => (int)$payload->id];
    if (isset($payload->disk_usage_mb)) {
        // Simpan dalam format text "X MiB" agar konsisten dengan dump lama
        $mb = (float)$payload->disk_usage_mb;
        $fields[] = "disk_usage = :disk_usage";
        $params[':disk_usage'] = $mb . " MiB";
    }
    if (isset($payload->inodes)) {
        $fields[] = "inodes = :inodes";
        $params[':inodes'] = (int)$payload->inodes;
    }
    if (isset($payload->expiry_date)) {
        $fields[] = "expiry_date = :expiry";
        $params[':expiry'] = $payload->expiry_date;
    }
    if (empty($fields)) {
...
function updatePassword($pdo, $currentUser, $payload) {
    if (!isset($payload->id) || !isset($payload->newPassword)) {
        http_response_code(400);
        echo json_encode(['success' => false, 'message' => 'Missing user ID or newPassword.']);
        return;
    }

    $targetId = (int)$payload->id;
    $newPwd = (string)$payload->newPassword;
    $oldPwd = isset($payload->oldPassword) ? (string)$payload->oldPassword : null;

    // Superadmin dapat mengganti tanpa verifikasi old password
    $isSelfChange = $currentUser->id === $targetId;
    if ($currentUser->role !== 'superadmin' && !$isSelfChange) {
        http_response_code(403);
        echo json_encode(['success' => false, 'message' => 'Permission denied.']);
        return;
    }
    ...
    // Jika bukan superadmin, wajib verifikasi password lama (jika hash tersedia)
    if ($currentUser->role !== 'superadmin') {
        ...
        $isHashed = strlen($currentHash) > 0 && (substr($currentHash, 0, 4) === '$2y$' || substr($currentHash, 0, 4) === '$2a$');
        $valid = $isHashed ? password_verify($oldPwd, $currentHash) : ($oldPwd === $currentHash);
        if (!$valid) {
