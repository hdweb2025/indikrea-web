...
try {
    ...
    if (is_dir($uploadDir)) {
        ...
        if ($files && count($files) > 0) {
            ...
            $latestUploadLogo = '/api/uploads/' . basename($files[0]);
        }
    }
} catch (\Throwable $e) {}

$companyLogo = $settingsFromDb['companyLogo'] ?? '';
if (!$companyLogo || !is_string($companyLogo) || trim($companyLogo) === '') {
    $companyLogo = $latestUploadLogo ?: '/icon.png';
}
